...
class OracleConfig:
    ...
    # oracle-plus-v1 toggles. None = use plus defaults (on). False/True = explicit.
    one_ply: bool | None = None
    solvency: bool | None = None
    denial: bool | None = None
    completing_trade: bool | None = None
    denial_weight: float = 1.0
    auction: bool | None = None
    inncenta_trade: bool | None = None
    networth_mix: float = 0.0
    auction_kind: str = "inncenta"
    family_body: str = "one_ply"
    one_ply_trades: bool | None = None
    phase_switch: bool | None = None
    cash_gate: bool | None = None
    build_first: bool | None = None
    race_buy: bool | None = None
    lethal_jail: bool | None = None
...
def oracle_config_from_args(args: Any) -> OracleConfig:
    ...
    return OracleConfig(
        ...
        one_ply=getattr(args, "one_ply", None),
        solvency=getattr(args, "solvency", None),
        denial=getattr(args, "denial", None),
        completing_trade=getattr(args, "completing_trade", None),
        denial_weight=float(getattr(args, "denial_weight", 1.0) or 1.0),
        auction=getattr(args, "auction", None),
        inncenta_trade=getattr(args, "inncenta_trade", None),
        networth_mix=float(getattr(args, "networth_mix", 0.0) or 0.0),
        auction_kind=str(getattr(args, "auction_kind", "inncenta") or "inncenta"),
        family_body="one_ply",
        one_ply_trades=getattr(args, "one_ply_trades", None),
        phase_switch=None,
        cash_gate=getattr(args, "cash_gate", None),
        build_first=getattr(args, "build_first", None),
        race_buy=getattr(args, "race_buy", None),
        lethal_jail=getattr(args, "lethal_jail", None),
    )
