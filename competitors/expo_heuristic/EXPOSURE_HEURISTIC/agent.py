"""
EXPO-v1: a net-worth-gradient heuristic for the ``ppo-plus-v2`` simulator.

Design note (why this is not ASU)
---------------------------------
ASU's reconstruction scores a state with ``M_assets + R_short + R_long +
M_monopoly``, where ``M_assets`` prices a deed at *list price plus paid
development cost* and cash is excluded, and it picks a move by cloning the
environment for every legal action and (for the rollout variant) running
truncated simulations.

EXPO does something structurally different.  It reads the simulator's own
win condition and optimises that directly.

``env.winner()`` resolves a capped game with ``Player.net_worth()``, and
``Property.calculate_net_worth()`` prices a deed at **2.5x list** while
unmortgaged, **5.0x list** once its colour group is complete, and a
developed lot at ``houses * house_price * (1 + 0.5 * houses)`` on top.
Cash counts at 1.0x.  Most games reach the 200-round cap, so that formula
-- not bankruptcy -- usually decides the match.

That gives an explicit exchange rate for every dollar:

    completing a colour group   ~9x  book value per dollar spent
    hotel on a 4-house lot      ~5.5x
    house                       ~3.0x
    any deed at list price      ~2.5x
    deed bought at 0.75x list   ~3.3x
    deed won at auction for $1  ~huge
    cash                         1.0x
    mortgaging a deed           -0.75x  (destroys book value)

So EXPO is a greedy ascent on

    U = NW_self - LAMBDA * NW_strongest_opponent
        + RENT_WEIGHT * expected_rent_flow
        - liquidity penalties

evaluated *analytically* per action family.  It never clones the
environment and never rolls out, which makes it far cheaper per decision
than the ASU value teacher.

Rent flow uses the exact stationary landing distribution of this engine's
board (``board_model.py``), not a uniform "28 deeds, 40/7 landings" prior,
so it correctly prices the orange/red traffic bulge downstream of jail.
"""
...
from .board_model import landing_odds, visits_per_lap
...
__all__ = ["ExpoHeuristicAgent", "POLICY_ID", "TUNABLE", "configure"]
